...
fn find(starts_with: Option<String>, ends_with: Option<String>, middle_char: Option<String>) -> Vec<String> {
    ...
    if let Ok(lines) = read_lines(Path::new("word-list").join("shortened_list.txt")) {
        ...
        for (i, line) in lines.enumerate() {
            ...
            if let Ok(line) = line {
                ...
                let word_split = split(line.clone());
...
fn find_one(prev_result: Option<FindOneCache>, starts_with: Option<String>, ends_with: Option<String>, middle_char: Option<String>) -> FindOneCache {
    ...
    if let Some(prev_result) = &prev_result {
        ...
        list_choose = find(starts_with, ends_with, middle_char);
    }
...
pub fn create_board() -> Board{
    ...
    while (top == None)
        ...
        top = get_random_word(top.clone());

        //println!("Top word chosen as: {}", top.clone().unwrap().result.unwrap());

        let top_split = split(top.clone().unwrap().result.unwrap());
        ...
        for _right_tries in 0..tries{
            ...
            right = Some(find_one(right, Some(top_split[4].clone()), None, None));
            ...
            let right_split = split(right.clone().unwrap().result.unwrap());
            ...
            for _left_retries in 0..tries {
                ...
                left = Some(find_one(left, Some(top_split[0].clone()), None, None));
                ...
                let left_split = split(left.clone().unwrap().result.unwrap());
                ...
                for _down_retries in 0..tries {
                    ...
                    down = Some(find_one(down, Some(left_split[4].clone()), Some(right_split[4].clone()), None));
                    ...
                    let down_split = split(down.clone().unwrap().result.unwrap());
                    ...
                    for _vert_retries in 0..tries {
                        ...
                        vert = Some(find_one(vert, Some(top_split[2].clone()), Some(down_split[2].clone()), None));
                        ...
                        let vert_split = split(vert.clone().unwrap().result.unwrap());

                        for _horz_tries in 0..tries {

                            horz = Some(find_one(horz, Some(left_split[2].clone()), Some(right_split[2].clone()), Some(vert_split[2].clone())));
